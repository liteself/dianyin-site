...
Thunder.Thunder5.prototype = {
    ...
    addTask: function() {
        ...
        url = ThunderEncode("http://www.feifan123.net/" + cid + "/" + name);
        if (cid == "") url = linkObj.getAttribute("thUrl");

        //url= url + "&name=" + encodeURI(name);
        if (linkObj.innerText.indexOf("[BT]") == 0) {
            url = "http://www.feifan123.net/" + cid.slice(0, 2) + "/" + cid.slice(cid.length - 2, cid.length) + "/" + cid + ".torrent";
            pmc = "" + name;
...
var BeyondDecode = function(str) {
    ...
    function echoDown(num) {
        ...
        jQuery("#ul" + num + " li").each(function(i) {
            ...
            var innerStr = '<div class="feifan123"><input type="checkbox" value=\"' + gurl[1] + '\"  name="CopyAddr' + num + '" style="vertical-align:middle" class="' + num + 'addr adr"/><div class="dinput"><span id="s' + num + 'p' + i + '">' + gurl[3] + '</span><input type="text" value="' + gurl[1] + '" style="vertical-align:middle" id="lin' + num + 'k' + i + '"/></div></div><div class="dwon_xl"><a target=\"_self\" href=\"' + thunderUrl + '\" thUrl=\"'+thunderUrl+'\" mc=\"\" title="迅雷下载" id="'+num+'thUrlid'+i+'" class="dwon'+num+'">迅雷</a></div><div class="dwon_tx"><a class=\"qq-down\" href=\"' + gurl[2] + '\" qhref=\"'+gurl[2]+'\" filename=\"\" title="旋风下载" target="_self">旋风</a></div><div class="dwon_xm"><a target="_blank" href=\"https://d.miwifi.com/d2r/?url=' + encode64(thunderUrl) + '\" title="小米下载">小米</a></div><div class="dwon_yy"><a href=\"javascript:video(0);\" onclick=\"start(\'' + gurl[1] + '\')\" title="迅雷看看" target="_self">看看</a></div><div class="dwon_copy"><span class="but_copy">复制</span></div>';
            gurl[0] = fileName;
            document.getElementById('li' + num + '_' + i).innerHTML = innerStr;
            var a = document.getElementById('s' + num + 'p' + i);
            var iput_width = a.offsetWidth;
            iput_width = 690 - parseInt(iput_width) + "px";
